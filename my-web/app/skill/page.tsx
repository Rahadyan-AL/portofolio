"use client";

import { MainLayout } from "@/components/layout/MainLayout";
import { skillClusters, type SkillItem } from "@/data/skills";
import { useI18n } from "@/lib/translations";

function hexForColor(color: string): string {
  if (color.includes("gold")) return "FFC93C";
  if (color.includes("pink")) return "FF2E93";
  if (color.includes("purple")) return "6B21D8";
  if (color.includes("cream")) return "F5F1E8";
  return "F5F1E8";
}

function FallbackIcon({ skill, iconColor }: { skill: SkillItem; iconColor: string }) {
  const fill = `#${iconColor}`;
  switch (skill.name) {
    case "Canva":
      return (
        <svg viewBox="0 0 24 24" className="h-[1em] w-[1em] shrink-0" fill={fill} aria-hidden style={{ filter: "drop-shadow(2px 2px 0 rgba(0,0,0,0.45))" }}>
          <title>Canva</title>
          <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zM6.962 7.68c.754 0 1.337.549 1.405 1.2.069.583-.171 1.097-.822 1.406-.343.171-.48.172-.549.069-.034-.069 0-.137.069-.206.617-.514.617-.926.548-1.508-.034-.378-.308-.618-.583-.618-1.2 0-2.914 2.674-2.674 4.629.103.754.549 1.646 1.509 1.646.308 0 .65-.103.96-.24.5-.264.799-.47 1.097-.8-.073-.885.704-2.046 1.851-2.046.515 0 .926.205.96.583.068.514-.377.582-.514.582s-.378-.034-.378-.17c-.034-.138.309-.07.275-.378-.035-.206-.24-.274-.446-.274-.72 0-1.131.994-1.029 1.611.035.275.172.549.447.549.205 0 .514-.31.617-.755.068-.308.343-.514.583-.514.102 0 .17.034.205.171v.138c-.034.137-.137.548-.102.651 0 .069.034.171.17.171.092 0 .436-.18.777-.459.117-.59.253-1.298.253-1.357.034-.24.137-.48.617-.48.103 0 .171.034.205.171v.138l-.136.617c.445-.583 1.097-.994 1.508-.994.172 0 .309.102.309.274 0 .103 0 .274-.069.446-.137.377-.309.96-.412 1.474 0 .137.035.274.207.274.171 0 .685-.206 1.096-.754l.007-.004c-.002-.068-.007-.134-.007-.202 0-.411.035-.754.104-.994.068-.274.411-.514.617-.514.103 0 .205.069.205.171 0 .035 0 .103-.034.137-.137.446-.24.857-.24 1.269 0 .24.034.582.102.788 0 .034.035.069.07.069.068 0 .548-.445.89-1.028-.308-.206-.48-.549-.48-.96 0-.72.446-1.097.858-1.097.343 0 .617.24.617.72 0 .308-.103.65-.274.96h.102a.77.77 0 0 0 .584-.24.293.293 0 0 1 .134-.117c.335-.425.83-.74 1.41-.74.48 0 .924.205.959.582.068.515-.378.618-.515.618l-.002-.002c-.138 0-.377-.035-.377-.172 0-.137.309-.068.274-.376-.034-.206-.24-.275-.446-.275-.686 0-1.13.891-1.028 1.611.034.275.171.583.445.583.206 0 .515-.308.652-.754.068-.274.343-.514.583-.514.103 0 .17.034.205.171 0 .069 0 .206-.137.652-.17.308-.171.48-.137.617.034.274.171.48.309.583.034.034.068.102.068.102 0 .069-.034.138-.137.138-.034 0-.068 0-.103-.035-.514-.205-.72-.548-.789-.891-.205.24-.445.377-.72.377-.445 0-.89-.411-.96-.926a1.609 1.609 0 0 1 .075-.649c-.203.13-.422.203-.623.203h-.17c-.447.652-.927 1.098-1.27 1.303a.896.896 0 0 1-.377.104c-.068 0-.171-.035-.205-.104-.095-.152-.156-.392-.193-.667-.481.527-1.145.805-1.453.805-.343 0-.548-.206-.582-.55v-.376c.102-.754.377-1.2.377-1.337a.074.074 0 0 0-.069-.07c-.24 0-1.028.824-1.166 1.373l-.103.445c-.068.309-.377.515-.582.515-.103 0-.172-.035-.206-.172v-.137l.046-.233c-.435.31-.87.508-1.075.508-.308 0-.48-.172-.514-.412-.206.274-.445.412-.754.412-.352 0-.696-.24-.862-.593-.244.275-.523.553-.852.764-.48.309-1.028.549-1.68.549-.582 0-1.097-.309-1.371-.583-.412-.377-.651-.96-.686-1.509-.205-1.68.823-3.84 2.4-4.8.378-.205.755-.343 1.132-.343zm9.77 3.291c-.104 0-.172.172-.172.343 0 .274.137.583.309.755a1.74 1.74 0 0 0 .102-.583c0-.343-.137-.515-.24-.515z" />
        </svg>
      );
    case "VS Code":
      return (
        <svg viewBox="0 0 24 24" className="h-[1em] w-[1em] shrink-0" fill={fill} aria-hidden style={{ filter: "drop-shadow(2px 2px 0 rgba(0,0,0,0.45))" }}>
          <title>Visual Studio Code</title>
          <path d="M23.15 2.587L18.21.21a1.494 1.494 0 0 0-1.705.29l-9.46 8.63-4.12-3.128a.999.999 0 0 0-1.276.057L.327 7.261A1 1 0 0 0 .326 8.74L3.899 12 .326 15.26a1 1 0 0 0 .001 1.479L1.65 17.94a.999.999 0 0 0 1.276.057l4.12-3.128 9.46 8.63a1.492 1.492 0 0 0 1.704.29l4.942-2.377A1.5 1.5 0 0 0 24 20.06V3.939a1.5 1.5 0 0 0-.85-1.352zm-5.146 14.861L10.826 12l7.178-5.448v10.896z" />
        </svg>
      );
    case "Matplotlib":
      return (
        <svg viewBox="0 0 24 24" className="h-[1em] w-[1em] shrink-0" fill="none" stroke={fill} strokeWidth={1.9} aria-hidden style={{ filter: "drop-shadow(2px 2px 0 rgba(0,0,0,0.45))" }}>
          <path d="M4 20V4M4 20h16" strokeLinecap="round" />
          <rect x="7" y="11" width="3" height="7" rx={0.7} fill={fill} stroke="none" />
          <rect x="11.5" y="7" width="3" height="11" rx={0.7} fill={fill} stroke="none" />
          <rect x="16" y="13" width="3" height="5" rx={0.7} fill={fill} stroke="none" />
        </svg>
      );
    case "Seaborn":
      return (
        <svg viewBox="0 0 24 24" className="h-[1em] w-[1em] shrink-0" fill="none" stroke={fill} strokeWidth={1.9} aria-hidden style={{ filter: "drop-shadow(2px 2px 0 rgba(0,0,0,0.45))" }}>
          <path d="M4 18c2-3 4-6 7-7 2-1 4 1 5 3 1 2 3 1 4-2" strokeLinecap="round" strokeLinejoin="round" />
          <circle cx={7} cy={12} r={1.3} fill={fill} stroke="none" />
          <circle cx={12} cy={9} r={1.3} fill={fill} stroke="none" />
          <circle cx={17} cy={12} r={1.3} fill={fill} stroke="none" />
        </svg>
      );
    case "Whimsical":
      return (
        <svg viewBox="0 0 24 24" className="h-[1em] w-[1em] shrink-0" fill="none" stroke={fill} strokeWidth={1.8} aria-hidden style={{ filter: "drop-shadow(2px 2px 0 rgba(0,0,0,0.45))" }}>
          <rect x={3} y={3} width={18} height={18} rx={2} />
          <path d="M7 8h4v4H7zM13 8h4v3h-4zM7 14h10v3H7z" fill={fill} stroke="none" opacity={0.95} />
        </svg>
      );
    default:
      return null;
  }
}

function SkillIcon({ skill }: { skill: SkillItem }) {
  const iconColor = hexForColor(skill.color);

  if (skill.iconSlug) {
    return (
      <img
        src={`https://cdn.simpleicons.org/${skill.iconSlug}/${iconColor}`}
        alt={`${skill.name} logo`}
        className="h-[1em] w-[1em] shrink-0"
        style={{ filter: "drop-shadow(2px 2px 0 rgba(0,0,0,0.45))" }}
        loading="lazy"
        onError={(e) => {
          (e.currentTarget as HTMLImageElement).style.display = "none";
        }}
      />
    );
  }

  return <FallbackIcon skill={skill} iconColor={iconColor} />;
}

export default function SkillPage() {
  const { t } = useI18n();

  return (
    <MainLayout>
      <div className="mx-auto max-w-[1100px] px-10 pt-[48px] pb-[80px] max-[900px]:px-6 max-[900px]:pt-6 max-[900px]:pb-8">
        <span
          className="text-[13px] tracking-[2px] uppercase text-[var(--pink)]"
          style={{ fontFamily: "var(--font-jetbrains), monospace" }}
        >
          {t("skill.kicker")}
        </span>
        <h1
          className="graffiti-heading mt-2 text-[clamp(34px,5vw,56px)]"
          style={{ fontFamily: "var(--font-bungee), cursive" }}
        >
          {t("skill.heading")}
        </h1>

        <div className="mt-10">
          {skillClusters.map((cluster) => (
            <div key={cluster.label} className="mb-11">
              <div
                className="mb-3.5 text-xs font-semibold tracking-[3px] text-white/80"
                style={{ fontFamily: "var(--font-jetbrains), monospace" }}
              >
                {cluster.label}
              </div>
              <div className="flex flex-wrap items-baseline gap-x-[22px] gap-y-1.5">
                {skillClusters.length > 0 &&
                  cluster.items.map((skill) => (
                    <span
                      key={skill.name}
                      className="relative inline-flex items-center gap-[0.22em] cursor-default select-none"
                      style={{
                        fontFamily: "var(--font-bungee), cursive",
                        fontSize: skill.size,
                        color: skill.color,
                        transform: `rotate(${skill.rotate})`,
                        textShadow: "2px 2px 0 rgba(0,0,0,0.45)",
                      }}
                    >
                      <SkillIcon skill={skill} />
                      {skill.name}
                      <span
                        className="absolute -bottom-1 left-[-2px] h-[6px] w-[85%] rounded-[40%_60%_55%_45%_/_60%_40%_60%_40%] opacity-50"
                        style={{ background: skill.color }}
                      />
                    </span>
                  ))}
              </div>
            </div>
          ))}
        </div>
      </div>
    </MainLayout>
  );
}
